...
data_x = pd.read_excel('../data/data.xlsx', sheet_name='metals')
data_y = pd.read_excel('../data/data.xlsx', sheet_name='overpotential')
seed = 0
...
nn_path = '../C2P/model/C2P_Model'
nn = torch.load(f'{nn_path}/model.pth')
norm_nn = joblib.load(f'{nn_path}/norm.pkl')
nn.eval()
with torch.no_grad():
    preds.append(norm_nn.inverse_transform(nn(torch.Tensor(x_test.values).cuda()).cpu().numpy()).ravel())

index = [*pull()['Model'], 'Neural Network']
...
pd.DataFrame(np.array([corr, mae, rmse, r2]).T, columns=['r', 'MAE', 'RMSE', 'R2'], index=index
             ).sort_values('r', ascending=False).to_excel('c2p_benchmark.xlsx')
